											...
											foreach($xml->files->children() as $folder){
												if(isset($folder['installationPath'])){
													...
													if(!CFile::copyDirectory($src, $dest)){
														$modulesError = true;
														$this->view->actionMessage .= CWidget::create('CMessage', array('error', A::t('core', 'An error occurred while copying the folder {source} to {destination}.', array('{source}'=>$src, '{destination}'=>$dest))));
													}
												}
											}
											if($modulesError){
