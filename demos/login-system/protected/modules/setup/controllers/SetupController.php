													...
													if(isset($folder['installationPath'])){
														...
														if(isset($folder['byDirectory']) && $folder['byDirectory'] == "true"){
															...
															if(!CFile::copyDirectory($srcFolder, $destFolder)){
																$modulesError = true;								
																$this->_view->actionMessage .= CWidget::create('CMessage', array('error', A::t('core', 'An error occurred while copying the folder {source} to {destination}.', array('{source}'=>$srcFolder, '{destination}'=>$destFolder))));
															}														
														}else{
																...
																foreach($folder->children() as $file){
																	...
																	if($installable){
																		...
																		if(!CFile::copyFile($src.$file, $dest.$file)){
																			$modulesError = true;
																			$this->_view->actionMessage .= CWidget::create('CMessage', array('error', A::t('core', 'An error occurred while copying the file {source} to {destination}.', array('{source}'=>$file, '{destination}'=>$destPath.'/'.$file))));
																		}
																	}
																}
															}														
