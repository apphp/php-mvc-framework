...
class CWidget
{
    ...
    public static function create($className, $params = [])
    {
		...
		if (!CClass::isExists($className)) {
            CDebug::addMessage('warnings', 'missing-helper', A::t('core', 'Cannot find widget class: {class}', ['{class}' => $className]));
        } else {
			// Init
			if (strtolower($className) == 'cmessage') {
				$type = isset($params[0]) ? $params[0] : '';
				$text = isset($params[1]) ? $params[1] : '';
                $init_params = isset($params[2]) ? $params[2] : [];
